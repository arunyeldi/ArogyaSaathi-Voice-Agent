...
import { Activity, AlertCircle, Brain, CheckCircle2, Loader2, Mic, Volume2 } from 'lucide-react';
...
export function ArogyaSaathiAgentStatus({ state, onStartAgain }: AgentStatusProps) {
    ...
    switch (state) {
      ...
      case 'checking':
        return {
          ...
          spin: true,
          bgClass:
            'bg-blue-50 text-blue-900 border-blue-300 dark:bg-blue-950/80 dark:text-blue-200 dark:border-blue-700',
      ...
      case 'thinking':
        return {
          ...
          spin: false,
          bgClass:
            'bg-indigo-50 text-indigo-900 border-indigo-300 dark:bg-indigo-950/80 dark:text-indigo-200 dark:border-indigo-700',
      ...
      case 'speaking':
        return {
          ...
          spin: false,
          bgClass:
            'bg-teal-50 text-teal-900 border-teal-300 dark:bg-teal-950/80 dark:text-teal-200 dark:border-teal-700',
  ...
  return (
    <div
      role="status"
      aria-live="polite"
      className={`inline-flex items-center gap-2.5 rounded-full border px-4 py-2 text-xs font-semibold shadow-md backdrop-blur-md transition-all duration-300 md:text-sm ${bgClass}`}
    >
      ...
      {state === 'ended' && onStartAgain && (
        <button
          ...
          className="ml-2 rounded-full bg-teal-600 px-3 py-1 text-xs font-semibold text-white shadow-xs transition-colors hover:bg-teal-700"
